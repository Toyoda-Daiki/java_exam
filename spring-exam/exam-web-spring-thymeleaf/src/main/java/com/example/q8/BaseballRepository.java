...
public class BaseballRepository {
	...
	private static final RowMapper<BaseballDomain> baseballDomainRowMapper = (rs, i) ->{
		...
		String stadiums = rs.getString("stadium_name");
		Date date = rs.getDate("established_date");
		return new BaseballDomain(id,name,stadiums,date);
		};
	
	public List<BaseballDomain> findAll(){
		String sql = "SELECT t.id, t.name, t.established_date, s.name AS stadium_name "
               + "FROM teams t "
